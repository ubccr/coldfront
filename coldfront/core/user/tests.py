...
class AdminTests(LiveServerTestCase):
    ...
    def test_admin(self):
        ...
        Alert(driver).accept()
        print("\nApproved allocation successfully.")

        #tests denying allocation request
        details_button = driver.find_elements_by_id("details_button")
        details_button[0].click()

        deny_button = driver.find_element_by_id("deny")
        deny_button.click()

        Alert(driver).accept()
        print("\nDenied allocation request successfully.")

        #tests approving allocation change request
        if (driver.find_element_by_id("hamburger_icon").is_displayed()):
            driver.find_element_by_id("hamburger_icon").click()

        driver.find_element_by_id("admin_dropdown").click()
        change_request_button = driver.find_element_by_id("navbar-allocation-change-requests")
        change_request_button.click()

        approve_change_button = driver.find_elements_by_id("change_approve")
        approve_change_button[0].click()
        print("\nApproved allocation change request successfully.")

        #tests denying allocation change request
        deny_change_button = driver.find_elements_by_id("change_details")
        deny_change_button[0].click()
