...
class Command(BaseCommand):
    ...
    def parse_input_file(file_path, allocation_period):
        ...
        partition the entries into three lists: (1) valid (User,
        Project) pairs, (2) Users that have already renewed, and (3)
        invalid input pairs."""
        ...
        for pi_name, project_name in pairs:
            try:
                project = Project.objects.prefetch_related(
                    'projectuser_set').get(name=project_name)
            except Project.DoesNotExist:
                invalid.append((pi_name, project_name))
                continue
    ...
    def process_invalid_pairs(self, invalid_pairs):
        """Given a list of (pi_name, project_name) pairs, where the PI
        and/or the Project are not valid, write to stderr."""
        for pi_name, project_name in invalid_pairs:
            message = f'Invalid pair: ({pi_name}, {project_name}). Skipping.'
            self.stderr.write(self.style.ERROR(message))

    def process_valid_pairs(self, valid, allocation_period, dry_run):
        """Given a list of valid (User, Project) pairs, create an
        AllocationRenewalRequest under the given AllocationPeriod with
        status 'Complete', and write to stdout.
        ...
        for pi_user, project in valid:
            pi_str = (
                f'{pi_user.first_name} {pi_user.last_name} ({pi_user.email})')
            if dry_run:
                message = (
                    f'Would create pre-completed AllocationRenewalRequest for '
                    f'PI {pi_str}, Project {project.name}, and '
            ...
            try:
                ...
                message = (
                    ...
                    f'{pi_str} and Project {project.name}.')
                self.stderr.write(self.style.ERROR(message))
                ...
                message = (
                    ...
                    f'and AllocationPeriod {allocation_period.name}.')
