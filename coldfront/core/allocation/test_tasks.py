...
from coldfront.core.allocation.models import Allocation, AllocationStatusChoice, AllocationAttributeType, \
    AllocationAttribute, AttributeType
from coldfront.core.allocation.tasks import send_expiring_mails, send_expired_mails, send_expiry_emails
...
class SendExpiringMailsTest(TestCase):
    def setUp(self):
        ...
        self.approver_profile = ApproverProfile.objects.create(user_profile=self.approver_user_profile)
        self.approver_profile.schools.add(self.school)

        # Approver for unrelated school
        self.other_approver_mail = 'approver2@example.com'
        self.other_user = UserFactory(username='approver2', email=self.other_approver_mail)
        self.other_user_profile = UserProfile.objects.get(user=self.other_user)
        self.other_approver_profile = ApproverProfile.objects.create(user_profile=self.other_user_profile)
        ...
        self.expire_attr_type = AllocationAttributeType.objects.create(name='EXPIRE NOTIFICATION',
                                                                       attribute_type=self.attribute_type,
                                                                       has_usage=False, is_private=True)
        self.cloud_attr_type = AllocationAttributeType.objects.create(name='CLOUD USAGE NOTIFICATION',
                                                                       attribute_type=self.attribute_type,
                                                                       has_usage=False, is_private=True)
...
class SendExpiredMailsTest(TestCase):
    ...
    @patch('coldfront.core.allocation.tasks.send_email_template')
    @patch('coldfront.core.allocation.tasks.EMAIL_ADMINS_ON_ALLOCATION_EXPIRE', new=False)
