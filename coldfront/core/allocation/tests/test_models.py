...
class AllocationModelGetInformationTests(TestCase):
    def test_is_safe_string(self):
        """Test that the is_safe_string method returns True for a safe string."""
        allocation: Allocation = AllocationFactory()
        self.assertIsInstance(allocation.get_information, SafeString)

    def test_malicious_html_input_is_sanitized(self):
        """Test that the get_information method returns sanitized output if allocation was made with unsafe input."""
        malicious_text: str = "<script>alert('You have been hacked!');</script>"
        ...
