...
class AllocationForm(forms.Form):
    ...
    def __init__(self, request_user, project_pk, *args, **kwargs):
        ...
        self.fields["resource"].queryset = get_user_resources(request_user).order_by(
            Lower("name")
        )
        ...
        if user_query_set:
            ...
            self.fields["users"].help_text = (
                "<br/>Select users in your project to add to this allocation."
            )
        else:
        ...
        if ALLOCATION_ACCOUNT_ENABLED:
            ...
            self.fields["allocation_account"].help_text = (
                '<br/>Select account name to associate with resource. <a href="#Modal" id="modal_link">Click here to create an account name!</a>'
            )
        else:
            self.fields["allocation_account"].widget = forms.HiddenInput()

        self.fields["justification"].help_text = (
            "<br/>Justification for requesting this allocation."
        )
...
class AllocationUpdateForm(forms.Form):
    ...
    status = forms.ModelChoiceField(
        queryset=AllocationStatusChoice.objects.exclude(
            name__in=[
                "Payment Pending",
                "Payment Requested",
                "Payment Declined",
...
class AllocationAttributeUpdateForm(forms.Form):
    ...
    def clean(self):
        cleaned_data = super().clean()
        allocation_attribute = AllocationAttribute.objects.get(
            pk=cleaned_data.get("attribute_pk")
        )
