...
class EmailUtilsTests(TestCase):
    ...
    def test_send_admin_email_template_with_receiver(self, mock_send_email_template):
        ...
        send_admin_email_template(
            "Test Subject", "template.txt", context, receiver_list=receivers
        )
    ...
    @patch("coldfront.core.utils.mail.send_email_template")
    def test_send_admin_email_template_with_default_receiver(
        self, mock_send_email_template
    ):
        context = {
            "pi": "Test PI",
            "resource": "Test Resource",
...
class SendAllocationAdminEmailTests(TestCase):
    def setUp(self):
        ...
        self.other_school_user = UserFactory(
            username="other_school_approver", email=self.other_school_approver_mail
        )
        self.other_school_user_profile = UserProfile.objects.get(
            user=self.other_school_user
        )
        self.other_school_approver_profile = ApproverProfile.objects.create(
            user_profile=self.other_school_user_profile
        )
    ...
    def test_send_allocation_admin_email_sends_to_matching_approvers(self, mock_send):
        ...
        with patch("coldfront.core.utils.mail.build_link", return_value=mock_link):
            with patch(
                "coldfront.core.utils.mail.email_template_context", return_value={}
            ):
                send_allocation_admin_email(
                    self.allocation, "Test Subject", "template.txt"
                )
        ...
        self.assertEqual(
            template_context["pi"],
            f"{self.pi.first_name} {self.pi.last_name} ({self.pi.username})",
        )
        self.assertEqual(template_context["resource"], self.resource)
        self.assertEqual(template_context["url"], mock_link)

    @patch("coldfront.core.utils.mail.send_admin_email_template")
    def test_send_allocation_admin_email_skips_if_no_matching_approvers(
        self, mock_send
    ):
    ...
    def test_send_allocation_admin_email_includes_project_manager(self, mock_send):
        ...
        ProjectUser.objects.create(
            user=manager_user, project=self.project, role=role, status=status
        )
