            ...
            if (not allocations.exists()): # under my project there are no allocations
                ...
                if (allocation): # get allocation
                    ...
                    for json_user in user_json_dict:
                        ...
                        for allocation_user in allocation_users:
                            print("finding username from allocation_users set")
                            if (allocation_user.user.username == json_user): # found user
                                print(json_user, "is in my ", allocation_users, "set")
                                # update allocationuser object

                            else:
                                print(json_user, "is not in my ", allocation_users, "set")
                                # if user object don't have json user, create
                                # else: skip creating user object
                                existance = User.objects.filter(username=json_user)
                                print("this user exists?",existance)
                                print(type(existance))
                                if (not existance.exists()):
                                    print("user doesn't exist, creating user object")
                                    user_obj = User.objects.create(
                                        username = json_user,
                                        first_name = user_json_dict[json_user]['name'], # debug later
                                        last_name = user_json_dict[json_user]['name'], # debug later 
                                        email = "Not_Active@fas.edu",
                                        is_active = False,
                                        is_staff = False,
                                        is_superuser = False,
                                    )
                                print("creating allocationuser object")
                                ...
                                # user_allocations = AllocationUser.objects.get(user=json_user)
                                # if (not user_allocations.exists()):
                                #     print("allocationuser doesn't exist thus creating one")
                                #     allocation_user_obj = AllocationUser.objects.create(
                                #         allocation=allocation,
                                #         user=User.objects.get(username=json_user),
                                #         # status=AllocationUserStatusChoice.objects.get(name='Inactive'),
