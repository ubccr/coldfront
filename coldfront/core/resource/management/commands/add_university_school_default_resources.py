...
DEFAULT_SCHOOL_RESOURCES_JSON_PATH = os.path.join(
    os.path.dirname(__file__), "..", "data", "school_resources.json"
)
...
class Command(BaseCommand):
    ...
    def add_general_university_resource(self):
        ...
        if created:
            logger.info(f"{GENERAL_RESOURCE_NAME} HPC cluster resource added.")
        else:
            logger.info(
                f"{GENERAL_RESOURCE_NAME} HPC cluster resource already available in DB."
            )
    ...
    def add_school_resources(self, json_file_path=DEFAULT_SCHOOL_RESOURCES_JSON_PATH):
        ...
        for rec in resources:
            ...
            parent_resource_obj = Resource.objects.get(name=GENERAL_RESOURCE_NAME)
            ...
            if created:
                logger.info(f" {rec['name']} resource added.")
            else:
                logger.info(f" {rec['name']} resource already available in DB.")
