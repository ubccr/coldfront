...
class ResourceDetailView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_child_resources(self, resource_obj):
        child_resources = [
            resource
            for resource in resource_obj.resource_set.all().order_by(Lower("name"))
        ]

        child_resources = [
            {
                "object": resource,
                "WarrantyExpirationDate": resource.get_attribute(
    ...
    def get_context_data(self, **kwargs):
        ...
        attributes = [
            attribute
            for attribute in resource_obj.resourceattribute_set.all().order_by(
...
class ResourceAttributeDeleteView(
    ...
    def test_func(self):
        ...
        if self.request.user.is_superuser:
            ...
            messages.error(
                self.request,
                "You do not have permission to delete resource attributes.",
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if resource_attributes_to_delete:
            formset = formset_factory(
                ResourceAttributeDeleteForm, max_num=len(resource_attributes_to_delete)
            )
            formset = formset(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            messages.success(
                request,
                "Deleted {} attributes from resource.".format(attributes_deleted_count),
            )
        else:
        ...
        if resource_search_form.is_valid():
            ...
            if data.get("installDate"):
                resources = resources.filter(
                    ...
                    & Q(
                        resourceattribute__value=data.get("installDate").strftime(
                            "%m/%d/%Y"
                        )
                    )
                )
            if data.get("serviceStart"):
                resources = resources.filter(
                    ...
                    & Q(
                        resourceattribute__value=data.get("serviceStart").strftime(
                            "%m/%d/%Y"
                        )
                    )
                )
            if data.get("serviceEnd"):
                resources = resources.filter(
                    ...
                    & Q(
                        resourceattribute__value=data.get("serviceEnd").strftime(
                            "%m/%d/%Y"
                        )
                    )
                )
            if data.get("warrantyExpirationDate"):
                resources = resources.filter(
                    Q(
                        resourceattribute__resource_attribute_type__name="WarrantyExpirationDate"
                    )
                    & Q(
                        resourceattribute__value=data.get(
                            "warrantyExpirationDate"
                        ).strftime("%m/%d/%Y")
                    )
                )
