...
class TestAllocationRenewalRequestReviewMemorandumSignedView(TestBase):
    """A class for testing
    ...
    @staticmethod
    def review_memorandum_signed_url(pk):
        """Return the URL for the view for reviewing the MOU status of
        the AllocationAdditionRequest with the given primary key."""
        return reverse(
    ...
    def test_permissions_get(self):
        ...
        requests."""
        url = self.review_memorandum_signed_url(
    ...
    def test_post_updates_request_state(self):
        ...
        # Test setting the status to 'Complete'.
        ...
        self.assertIn(
            'has been set to Complete', self.get_message_strings(response)[0])
        ...
        memorandum_signed = self.allocation_addition_request.state[
            'memorandum_signed']
        self.assertEqual(memorandum_signed['status'], data['status'])
