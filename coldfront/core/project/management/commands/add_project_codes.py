...
class Command(BaseCommand):
    ...
    def update_project_code(self, projects):
        user_input = input('Assign all existing projects with project codes? You can use the --dry-run flag to preview changes first. [y/N] ')
        ...
        try:
            ...
            self.stdout.write('Error, no changes made. Please set PROJECT_CODE as a string value inside configuration file.')

    def project_code_dry_run(self, projects):
        try:
            for project in projects:
                new_code = generate_project_code(PROJECT_CODE, project.pk, PROJECT_CODE_PADDING)
                self.stdout.write(f"Project {project.pk}, called {project.title}: new project_code would be '{new_code}'")
        except AttributeError:
            self.stdout.write('Error, no changes made. Please set PROJECT_CODE as a string value inside configuration file.')
