...
class Command(BaseCommand):
    ...
    def update_project_institution(self, projects):
        ...
        user_input = input('Assign all existing projects with institutions? You can use the --dry-run flag to preview changes first. [y/N] ')
    ...
    def institution_dry_run(self, projects):
        ...
        try:
            for project in projects:
                ...
                self.stdout.write(f"Project {project.pk}, called {project.title}. Institution would be '{new_institution}'")
        except Exception as e:
            self.stdout.write(f'Error: {e}')
