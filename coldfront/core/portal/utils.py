...
def generate_publication_by_year_chart_data(publications_by_year):
    if publications_by_year:
        ...
        data = {
            "x": "Year",
            "columns": [years, publications],
            "type": "bar",
            "colors": {"Publications": "#17a2b8"},
        }
    else:
...
def generate_resources_chart_data(allocations_count_by_resource_type):
    if allocations_count_by_resource_type:
        cluster_label = "Cluster: %d" % (
            allocations_count_by_resource_type.get("Cluster", 0)
        )
        cloud_label = "Cloud: %d" % (allocations_count_by_resource_type.get("Cloud", 0))
        server_label = "Server: %d" % (
            allocations_count_by_resource_type.get("Server", 0)
        )
        storage_label = "Storage: %d" % (
            allocations_count_by_resource_type.get("Storage", 0)
        )
...
def generate_allocations_chart_data():
    ...
    renewal_requested_count = Allocation.objects.filter(
    ...
    expired_count = Allocation.objects.filter(
        status__name="Expired", end_date__gte=start_time
    ).count()
