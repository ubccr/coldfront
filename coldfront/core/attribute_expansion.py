...
def get_attribute_parameter_value(
    argument, attribute_parameter_dict, error_text, resources=[], allocations=[]
):
    ...
    try:
        ...
        if opcode == "(":
            if argument == "floor":
                ...
                logger.error(
                    "Unrecognized function named {} in {}= for "
                    "{}, returning None".format(argument, opcode, error_text)
                )
                return None
        # If reached here, we do not recognize opcode
        logger.error(
            "Unrecognized operation {}= in {}, returning None".format(
                opcode, error_text
            )
        )
    except Exception:
...
def process_attribute_parameter_string(
    ...
    if opcode == "(":
        ...
        error_text = (
            "processing attribute_parameter_string={pstr} "
...
def make_attribute_parameter_dictionary(
    attribute_name, attribute_parameter_string, resources=[], allocations=[]
):
...
def expand_attribute(
    raw_value, attribute_name, attriblist_string, resources=[], allocations=[]
):
    ...
    try:
        ...
        # return raw_value
        logger.error(
            "Error expanding {aname}: {error}".format(aname=attribute_name, error=xcept)
        )
        return raw_value
...
def convert_type(value, type_name, error_text="unknown"):
    ...
    if type_name.endswith("Text"):
        try:
            ...
            logger.error(
                'Error converting "{}" to {} in {}'.format(value, "Text", error_text)
            )
            return value
    ...
    if type_name.endswith("Int"):
        try:
            ...
            logger.error(
                'Error converting "{}" to {} in {}'.format(value, "Int", error_text)
            )
            return value
    ...
    if type_name.endswith("Float"):
        try:
            ...
            logger.error(
                'Error converting "{}" to {} in {}'.format(value, "Float", error_text)
            )
            return value
