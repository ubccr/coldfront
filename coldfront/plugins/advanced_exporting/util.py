...
class TableBuilder:
    def build_table(self, data, allocationattribute_data, get_request):
        if data.get('only_search_projects'):
            queryset = self.build_project_queryset(data, get_request)
            columns = self.build_project_columns(data)
            rows = self.build_project_rows(columns, queryset)
            return rows, columns
        
        queryset = self.build_queryset(data, get_request)
        columns = self.build_columns(data, allocationattribute_data)
        additional_data = self.get_allocation_attribute_data(allocationattribute_data)
        rows = self.build_rows(columns, queryset, additional_data)

        return rows, columns
    
    def build_columns(self, data, allocationattribute_data):
        columns = []
    ...
    def build_rows(self, columns, queryset, additional_data):
        rows_dict = {}
    ...
    def build_queryset(self, data, request):
        allocation_queryset = self.build_allocation_queryset(data, request)

        project_queryset = self.build_project_queryset(data, request)
    ...
    def get_allocation_attribute_data(self, data):
    ...
    def build_allocation_queryset(self, data, request):
    ...
    def build_project_queryset(self, data, request):
        order_by = 'id'
        if data.get('only_search_projects'):
            order_by = request.get('order_by')
            if order_by:
    ...
    def build_project_rows(self, columns, queryset):
