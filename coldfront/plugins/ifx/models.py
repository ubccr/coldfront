...
def allocation_user_to_allocation_product_usage(allocation_user, product, overwrite=False):
    ...
    product_usage_data['description'] = f"{product_usage_data['quantity']} {product_usage_data['units']} of {allocation_user.allocation.get_attribute('Storage Quota (TB)')} allocation of {product_usage_data['product']} for {product_usage_data['product_user']} on {product_usage_data['start_date']}"
