...
SLURM_CMD_REMOVE_USER = (
    SLURM_SACCTMGR_PATH + " -Q -i delete user where name={} cluster={} account={}"
)
SLURM_CMD_REMOVE_QOS = (
    SLURM_SACCTMGR_PATH
    + " -Q -i modify user where name={} cluster={} account={} set {}"
)
SLURM_CMD_REMOVE_ACCOUNT = (
    SLURM_SACCTMGR_PATH + " -Q -i delete account where name={} cluster={}"
)
SLURM_CMD_ADD_ACCOUNT = SLURM_SACCTMGR_PATH + " -Q -i create account name={} cluster={}"
SLURM_CMD_ADD_USER = (
    SLURM_SACCTMGR_PATH + " -Q -i create user name={} cluster={} account={}"
)
SLURM_CMD_CHECK_ASSOCIATION = (
    SLURM_SACCTMGR_PATH
    + " list associations User={} Cluster={} Account={} Format=Cluster,Account,User,QOS -P"
)
SLURM_CMD_LIST_ACCOUNTS = (
    SLURM_SACCTMGR_PATH + " list associations User={} Cluster={} Format=Account -Pn"
)
SLURM_CMD_CHECK_DEFAULT_ACCOUNT = (
    SLURM_SACCTMGR_PATH + " show user User={} Cluster={} Format=DefaultAccount -Pn"
)
SLURM_CMD_CHANGE_DEFAULT_ACCOUNT = (
    SLURM_SACCTMGR_PATH
    + " -Q -i modify user User={} where Cluster={} set DefaultAccount={}"
)
SLURM_CMD_BLOCK_ACCOUNT = (
    SLURM_SACCTMGR_PATH
    + " -Q -i modify account {} where Cluster={} set GrpSubmitJobs=0"
)
...
def slurm_remove_assoc(user, cluster, account, noop=False):
    ...
    cmd = SLURM_CMD_CHECK_DEFAULT_ACCOUNT.format(
        shlex.quote(user), shlex.quote(cluster)
    )
