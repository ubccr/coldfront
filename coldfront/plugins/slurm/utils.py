...
SLURM_CMD_CHANGE_DEFAULT_ACCOUNT = SLURM_SACCTMGR_PATH + ' -Q -i modify user User={} set DefaultAccount={}'
...
def slurm_remove_assoc(user, cluster, account, noop=False):
    ...
    cmd = SLURM_CMD_CHECK_DEFAULT_ACCOUNT.format(shlex.quote(user))
    ...
    #get accounts
    cmd = SLURM_CMD_LIST_ACCOUNTS.format(shlex.quote(user))
    output = _run_slurm_cmd(cmd, noop=noop)
    accounts = []
    with StringIO(output.decode("UTF-8")) as fh:
        reader = fh.getvalue()
    ...
    for userAccount in accounts:
        if userAccount != account:
            cmd = SLURM_CMD_CHANGE_DEFAULT_ACCOUNT.format(shlex.quote(user), shlex.quote(userAccount))
