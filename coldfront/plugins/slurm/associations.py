...
class SlurmCluster(SlurmBase):
    ...
    def new_from_resource(resource):
        ...
        children = Resource.objects.filter(
            Q(parent_resource_id=resource.id),
            Q(resource_type__name="Generic") | Q(resource_type__name="Cluster"),
        )
        for r in children:
    ...
    def add_allocation(self, allocation, specs=None, user_specs=None):
        ...
        logger.debug(
            "Adding allocation name=%s specs=%s user_specs=%s", name, specs, user_specs
        )
    ...
    def write(self, out):
        self._write(
            out,
            "# ColdFront Allocation Slurm associations dump {}\n".format(
...
class SlurmAccount(SlurmBase):
    ...
    def new_from_sacctmgr(line):
        ...
        if not re.match("^Account - '[^']+'", line):
            raise (
                SlurmParserError(
                    'Invalid format. Must start with "Account" for line: {}'.format(
                        line
                    )
                )
            )
    ...
    def add_allocation(self, allocation, user_specs=None):
        ...
        if name != self.name:
            raise (
                SlurmError(
                    "Allocation {} slurm_account_name does not match {}".format(
        ...
        allocation_user_specs = allocation.get_attribute_list(
            SLURM_USER_SPECS_ATTRIBUTE_NAME
        )
