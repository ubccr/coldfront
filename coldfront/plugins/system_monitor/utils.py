...
class SystemMonitor:
    ...
    def parse_html_using_beautiful_soup(self):
        ...
        try:
            processors_utilized = [
                int(ele.strip())
                for ele in tables_dict["utilization"][0]["Processors Utilized"].split(
                    "of"
                )
            ]
            jobs = [
                (ele.get("Running"), ele.get("Queued"))
                for ele in tables_dict["jobs"]
                if ele.get("Partition") == ""
            ][0]
            job_numbers = [int(ele.split()[0]) for ele in jobs]

            free = processors_utilized[1] - processors_utilized[0]
            utilized_percent = (
                round(processors_utilized[0] / processors_utilized[1] * 1000) / 10
            )
            free_percent = round(free / processors_utilized[1] * 1000) / 10

            utilized_label = "Processors Utilized: %s (%s%%)" % (
                processors_utilized[0],
                utilized_percent,
            )
