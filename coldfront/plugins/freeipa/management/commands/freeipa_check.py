    ...
    def disable_user_in_coldfront(self, user, freeipa_status):
        ...
        for ua in user_allocations:
            if ua.status.name == "Active" and ua.allocation.status.name == "Active":
                logger.info(
                    "Removing user from allocation user=%s allocation=%s",
                    user.username,
                    ua.allocation,
                )
        ...
        for pa in user_projects:
            if pa.status.name == "Active" and pa.project.status.name == "Active":
                logger.info(
                    "Removing user from project user=%s project=%s",
                    user.username,
                    pa.project,
                )
        ...
        if self.disable:
            for user in users:
                ...
                try:
                    ...
                    if (
                        "nsAccountLock" in result
                        and str(result["nsAccountLock"][0]) == "TRUE"
                    ):
                        # User is disabled in FreeIPA so disable in coldfront
                        logger.info(
                            "User is disabled in FreeIPA so disable in ColdFront: %s",
                            user.username,
                        )
                    ...
                    if "No such user" in str(e) or "NotFound" in str(e):
                        ...
                        logger.info(
                            "User is not found in FreeIPA so disable in ColdFront: %s",
                            user.username,
                        )
                        self.disable_user_in_coldfront(user, "NotFound")
                    else:
                        logger.error(
                            "dbus error failed while checking user %s in FreeIPA: %s",
                            user.username,
                            e,
                        )
