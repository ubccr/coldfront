...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for allocationuser in AllocationUser.objects.all():
            ...
            if (
                allocation.status.name != "Expired"
                and allocationuser.status.name == "Removed"
            ):
        ...
        for key in expired_allocation_users.keys():
            ...
            try:
                ...
                if (
                    ...
                    and str(result["nsAccountLock"][0]).lower() == "true"
                ):
                    ...
                    self.writerow(
                        [
                            key,
                            expired_allocation_users[key]["expire_date"].strftime(
                                "%Y-%m-%d"
                            ),
                            build_link(
                                reverse(
                                    "allocation-detail",
                                    kwargs={
                                        "pk": expired_allocation_users[key][
                                            "allocation_id"
                                        ]
                                    },
                                )
                ...
                if "No such user" in str(e) or "NotFound" in str(e):
                    ...
                    logger.error(
                        "dbus error failed to find user %s in FreeIPA: %s", key, e
                    )
