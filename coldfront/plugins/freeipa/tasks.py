...
def add_user_group(allocation_user_pk):
    ...
    for g in groups:
        ...
        try:
            ...
            logger.info(
                "Added user %s to group %s successfully",
                allocation_user.user.username,
                g,
            )
...
def remove_user_group(allocation_user_pk):
    ...
    for g in groups:
        ...
        try:
            ...
            logger.warning(
                "User %s is not a member of group %s", allocation_user.user.username, g
            )
        except Exception as e:
            logger.error(
                "Failed removing user %s from group %s: %s",
                allocation_user.user.username,
                g,
                e,
            )
            ...
            logger.info(
                "Removed user %s from group %s successfully",
                allocation_user.user.username,
                g,
            )
