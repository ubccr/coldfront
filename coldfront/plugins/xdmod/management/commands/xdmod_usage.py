...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-r", "--resource", help="Report usage data for specific Resource"
        )
        parser.add_argument(
            "-a", "--account", help="Report usage data for specific account"
        )
        parser.add_argument(
            "-u", "--username", help="Report usage for specific username"
        )
        parser.add_argument(
            "-p", "--project", help="Report usage for specific cloud project"
        )
        parser.add_argument(
            "-s",
            "--sync",
            help="Update allocation attributes with latest data from XDMoD",
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--expired",
            help="XDMoD statistic for archived projects",
            action="store_true",
        )
    ...
    def process_total_storage(self):
        ...
        if self.filter_account:
            allocations = allocations.filter(
                Q(
                    allocationattribute__allocation_attribute_type__name=XDMOD_STORAGE_GROUP_ATTRIBUTE_NAME
                )
        ...
        for s in allocations.distinct():
            ...
            if not account_name:
                logger.warn(
                    "%s attribute not found for allocation: %s",
                    XDMOD_STORAGE_GROUP_ATTRIBUTE_NAME,
                    s,
                )
                continue
            ...
            if len(resources) == 0:
                logger.warn(
                    "%s attribute not found on any resouces for allocation: %s",
                    XDMOD_RESOURCE_ATTRIBUTE_NAME,
                    s,
                )
                continue
    ...
    def process_total_gpu_hours(self):
        ...
        if self.filter_account:
            allocations = allocations.filter(
                Q(
                    allocationattribute__allocation_attribute_type__name=XDMOD_ACCOUNT_ATTRIBUTE_NAME
                )
                & Q(allocationattribute__value=self.filter_account)
            )
    ...
    def process_cloud_core_time(self):
        ...
        if self.filter_project:
            allocations = allocations.filter(
                Q(
                    allocationattribute__allocation_attribute_type__name=XDMOD_CLOUD_PROJECT_ATTRIBUTE_NAME
                )
