...
def handle_missing_project_in_openldap_archive(project, project_dn, sync=False, write_to_archive=False):
    ...
    try:
        ...
        print(f"Exception creating vars for OpenLDAP archive action for Project {project.project_code}: {e}")
    ...
    if sync and write_to_archive:
        try:
            # notify
            print(f"Adding archived project {archive_posixgroup_dn} to OpenLDAP - SYNC is {sync} - WRITING TO Openldap")
            ...
            # create posixgroup
            print(f"Adding OpenLDAP project archive posixgroup entry - DN: {archive_posixgroup_dn}")
            add_project_posixgroup_to_openldap(
...
def handle_project_in_openldap_but_not_archive(project, project_ou_dn, archive_dn, sync=False, write_to_archive=False):
    ...
    if sync and write_to_archive:
        ...
        try:
            ...
            print(f"Moving project to archive OU, DN: {archive_dn} in OpenLDAP - SYNC is {sync} - WRITING TO Openldap")
        except Exception as e:
            print(f"Exception moving project {project.project_code} to archive OU, DN: {archive_dn} in OpenLDAP: {e}")
...
def local_get_cf_django_members(project_pk):
    ...
    usernames = [user.user.username for user in queryset if user.user.username not in PROJECT_OPENLDAP_EXCLUDE_USERS]
...
def sync_members(
    ...
    if len(cf_members) == 0 and len(openldap_members) == 0:
        print("NOTIFICATION: Both Coldfront django and OpenLDAP have 0 members - no action to take by sync_members")
        return
    ...
    if ldapsearch_project_result and (len(project_dn) > 0) and not ldapsearch_project_result_archive:
        ...
        print("WARNING: sync_members NO ACTION PERFORMED, couldn't match ldapsearch result and provide a valid DN")
        ...
        if sync:
            if ldapsearch_project_result:
                try:
                    ...
                    print(f"Exception Removing members {missing_in_cf} in OpenLDAP DN {member_change_dn}: {e}")
    ...
    if len(missing_in_openldap) > 0:
        print(f"Users are MISSING in OpenLDAP - ADDITION ACTION (OpenLDAP): {member_change_dn}\n {missing_in_openldap}")
        if not sync:
            print("sync is required to make changes, please supply: -s or --sync")
        if sync:
            if ldapsearch_project_result:
                try:
                    ...
                    print(f"Exception Adding members {missing_in_openldap} in OpenLDAP DN {member_change_dn}: {e}")
            elif ldapsearch_project_result_archive:
                if not write_to_archive:
                    print("WRITE_TO_ARCHIVE is required to make changes, please supply: -z or --write_archive")
                elif write_to_archive:
...
def sync_check_project(
    ...
    if skip_archived and project.status_id in [PROJECT_STATUS_CHOICE_ARCHIVED]:
        print("--------------------")
        print(f"Requested skip_archived, not processing archived project status for Project {project.project_code}")
        print("")
        return
    ...
    if PROJECT_OPENLDAP_ARCHIVE_OU:
        ...
        print("search project archive OU result: N/A - PROJECT_OPENLDAP_ARCHIVE_OU is not set")
        if project.status_id in [PROJECT_STATUS_CHOICE_ARCHIVED]:
    ...
    if project.status_id in [PROJECT_STATUS_CHOICE_ARCHIVED]:
        ...
        if PROJECT_OPENLDAP_ARCHIVE_OU and PROJECT_OPENLDAP_REMOVE_PROJECT:
            ...
            if not ldapsearch_project_result and not ldapsearch_project_result_archive:
                handle_missing_project_in_openldap_archive(project, project_dn, sync, write_to_archive)
                return
        ...
        if PROJECT_OPENLDAP_ARCHIVE_OU:
            if ldapsearch_project_result_archive:
                print(f"Project {project.project_code} is an archived project - found {project_archive_dn}")
    ...
    # Project is new or active status in Coldfront django
        ...
        if not ldapsearch_project_result:
            ...
            print(f"Project {project.project_code} is a new or active project - found {project_dn}")
    else:
    ...
    if openldap_members is None:
        print("NOTIFICATION: openldap_members could not be determined ('None' value), possible causes include:")
        print("- project has been removed from OpenLDAP - as expected - archive OU is not set")
        print("- archived project in Coldfront no further action required...")
        print("- previously enabling OpenLDAP archive OU, then disabling, whilst having projects in the archive OU")
        return
...
def loop_all_projects(
    ...
    for project in projects:
        if hasattr(project, "project_code") and project.project_code:
            ...
            print("--------------------")
            print(f"Project with pk in Coldfront django {project.pk} - has no project_code")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["project_group"]:
            logger.info("Filtering output by project_group: %s", options["project_group"])
