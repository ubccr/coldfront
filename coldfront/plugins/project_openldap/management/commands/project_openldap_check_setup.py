...
def check_env_var_existance(name, expected_value, required=True):
    ...
    try:
        ...
        if not env_var and required:
            print(f"\033[31m [REQUIRED] WARNING - {name} is not set or 0 length in settings!\033[0m")
            return None

        if name == "PROJECT_CODE_PADDING" and int(env_var) == 0:
            print("\033[31m [OPTIONAL] WARNING PROJECT_CODE_PADDING is NOT VALID - example value: 4!\033[0m")
            return None
        ...
        if name == "PROJECT_OPENLDAP_DESCRIPTION_TITLE_LENGTH" and (int(env_var) <= 10 or int(env_var) >= 300):
            print(
        ...
        print(f"\033[32m{status} OK - {name} is {env_var if 'PASSWORD' not in name else 'set'}\033[0m")
    except ImportError:
...
def check_all_openldap_options_enabled():
    print("")
    print("\033[36m---------------------------------------------------\033[0m")
    print("\033[36m Check if all of required Project OpenLDAP imports are loaded in the configuration file. \033[0m")
    print("\033[36m---------------------------------------------------\033[0m")
    print("")
