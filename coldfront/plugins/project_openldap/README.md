...
Should Coldfront continue creating or modifying projects whilst the OpenLDAP server unavailable, corrective action will likely be needed. A managemnet command is provided to perform checks and synchronize changes required. This should only be used when Coldfront and OpenLDAP are (or are suspected) to be out of sync.
...
Also included within that directory is a Mermaid diagram (``Mermaid.md``) of the main function of the syncer.
...
- You will also need to supply an OU to store projects in and optionally an OU to store (move) archived projects to. You should create this in the nomral way e.g. ldif.
...
- **NOTE: OpenLDAP security and general site security is the responsability of site administrators.**
...
**NOTE:** Security (e.g. SSL + TLS) configuration is the responsability of the site administrator - these aren't production settings below, only a starting point.
...
- the syncer script may also see changes to push a couple of methods in ``utils.py`` - e.g. fetch membership.
...
Some sites may want per-insitution project OUs (which then contain the relevant project group OUs). Currently this is out-of-scope. The single projects OU is currently provided/favoured for simplicity.