...
from ldap3 import Connection, Server, Tls, MODIFY_ADD, MODIFY_DELETE, MODIFY_REPLACE
...
PROJECT_OPENLDAP_CONNECT_TIMEOUT = import_from_settings(
    "PROJECT_OPENLDAP_CONNECT_TIMEOUT"
)
...
def add_members_to_openldap_project_posixgroup(dn, list_memberuids, write=True):
    ...
    conn = openldap_connection(
        server, PROJECT_OPENLDAP_BIND_USER, PROJECT_OPENLDAP_BIND_PASSWORD
    )
...
def add_per_project_ou_to_openldap(
    project_obj, dn, openldap_ou_description, write=True
):
    ...
    try:
        ...
        logger.error("Atrributes to write...")
...
def add_project_posixgroup_to_openldap(dn, openldap_description, gid_int, write=True):
    ...
    try:
        ...
        logger.error("Atrributes to write...")
        logger.error(
            f"posixGroup description - {openldap_description} gidNumber - {gid_int}"
        )
        logger.error(exc_log)
    finally:
        conn.unbind()
...
def ldapsearch_check_project_ou(OU):
    ...
    try:
        ldapsearch_check_project_ou_result = conn.search(
...
def ldapsearch_get_project_description(dn):
    ...
    try:
        ...
        ldapsearch_project_description = ldapsearch_project_description_entries[
            0
        ].description
...
def allocate_project_openldap_gid(project_pk, PROJECT_OPENLDAP_GID_START):
    """Create a GID for use as gidNubmer in the project's posixGroup"""
    # add the GID start
...
def construct_dn_archived_str(project_obj):
    ...
    try:
        project_code_str = project_obj.project_code
        dn = (
            f"cn={project_code_str},ou={project_code_str},{PROJECT_OPENLDAP_ARCHIVE_OU}"
        )
        return dn
    except Exception as exc_log:
        logger.info(exc_log)
