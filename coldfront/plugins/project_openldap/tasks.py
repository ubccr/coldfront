...
def add_project(project_obj):
    ...
    if not hasattr(project_obj, "project_code"):
        logger.info(
            "WARNING: Enable project_code to use the project_openldap plugin to add projects into OpenLDAP"
        )
        logger.info(
    ...
    openldap_posixgroup_description = construct_project_posixgroup_description(
        project_obj
    )
    ...
    add_project_posixgroup_to_openldap(
        posixgroup_dn, openldap_posixgroup_description, gid_int
    )
...
def remove_project(project_obj):
    ...
    if PROJECT_OPENLDAP_REMOVE_PROJECT and not PROJECT_OPENLDAP_ARCHIVE_OU:
        ...
        posixgroup_dn = construct_dn_str(project_obj)
        logger.info(
            f"Project POSIXGROUP {posixgroup_dn} is going to be REMOVED from OpenLDAP..."
        )
