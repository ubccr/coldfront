...
def add_project(project_obj):
    ...
    if not hasattr(project_obj, "project_code"):
        logger.info("WARNING: Enable project_code to use the project_openldap plugin to add projects into OpenLDAP")
        logger.info(f"WARNING: Additional message - this issue was encountered with project pk {project_obj.pk}")
        return None
